...
app.post('/saveDeviceData',function(req,res){
...
app.post('/saveAppliancesData',function(req,res){
...
app.get('/login',function(req,res){
    ...
    var hashed_pass = crypto.createHash('sha512').update(password).digest("hex");
    var query = user.find({'user_id':req.query.id, 'user_password':hashed_pass});
   //  var query = user.find({'userEmail':req.body.email, 'userPassword':hashed_pass});
    query.select('user_name user_role');
    query.exec(function(err,data3){
        if(err) return handleError(err);
        res.send(JSON.stringify(data3[0]),);
        console.log(JSON.stringify(data3[0]));
    });
 /*  fs.readFile('./profile_Image/' + req.query.id +'.jpg', function(err, data){
        res.send(data.toString());
        console.log(data);
     }); */
}); 

app.post('/devices_list',function(req,res){
...
app.get('/devices_data_view',function(req,res){
...
app.get('/appliances_list',function(req,res){
...
app.get('/appliance_data_view',function(req,res){
...
app.post('/updateUserAccount',function(req,res){
